                        ...
                        {currentEpisodes.length > 0 ? currentEpisodes.map((ep, index) => {
                          ...
                          return (
                            <motion.div
                              ...
                              key={ep.id}
                              className={`group rounded-xl overflow-hidden bg-[#0f0f0f] border transition-all ${
                                isLocked ? 'border-white/5 opacity-70' : 'border-white/5 hover:border-gold/20'
                              }`}
                            >
                              <div className="flex items-stretch gap-0">
                                ...
                                <div
                                  ...
                                  className={`relative shrink-0 overflow-hidden ${isLocked ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                                ...
                                >
                                  ...
                                  {isLocked ? (
                                    ...
                                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 flex items-center justify-center transition-all">
                                      <div className="w-8 h-8 rounded-full bg-white/0 group-hover:bg-white/20 backdrop-blur-sm flex items-center justify-center transition-all">
                                        <Play size={14} fill="white" className="text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg" />
                                      </div>
                                    </div>
                                  )}
                                  {/* Ep number badge */}
                                  <div className="absolute bottom-1.5 left-1.5 bg-black/70 backdrop-blur-sm text-[9px] font-bold text-white px-1.5 py-0.5 rounded">
                                    E{ep.number}
                                  </div>
                                </div>
                                ...
                                <div className="flex-1 flex flex-col justify-between p-2.5 min-w-0">
                                  ...
                                  <div className="flex items-start justify-between gap-1 mb-1.5">
                                    <div className="min-w-0 flex-1">
                                      <h4 className={`text-xs font-bold leading-tight truncate ${isLocked ? 'text-gray-400' : 'text-gray-100 group-hover:text-gold transition-colors'}`}>
                                        {ep.number}. {ep.title}
                                      </h4>
                                      {isLocked && ep.releaseDate && (
                                        <div className="flex items-center gap-1 mt-0.5">
                                          <Calendar size={8} className="text-yellow-400" />
                                          <span className="text-[9px] text-yellow-400">{ep.releaseDate}</span>
                                        </div>
                                      )}
                                      {isLocked && !ep.releaseDate && (
                                        <span className="text-[9px] bg-yellow-500/20 text-yellow-400 px-1.5 py-0.5 rounded-full border border-yellow-500/30 mt-0.5 inline-block">
                                          COMING SOON
                                        </span>
                                      )}
                                    </div>
                                    {/* Duration - Enhanced Display */}
                                    {ep.duration && (
                                      <div className="flex items-center gap-1 text-[9px] font-semibold text-gray-400 bg-black/50 backdrop-blur-sm px-2 py-1 rounded-md shrink-0 border border-white/5">
                                  ...
                                  <div className="flex items-center gap-1.5 flex-wrap mb-2">
                                    {ep.quality && (
                                      <span className="flex items-center gap-0.5 text-[9px] text-blue-400 bg-blue-500/10 px-1.5 py-0.5 rounded border border-blue-500/20">
                                        <Tv size={7} />{ep.quality}
                                      </span>
                                    )}
                                    {ep.fileSize && (
                                      <span className="flex items-center gap-0.5 text-[9px] text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20">
                                        <Database size={7} />{ep.fileSize}
                                      </span>
                                    )}
                                    {ep.audioLanguage && (
                                      <span className="flex items-center gap-0.5 text-[9px] text-purple-400 bg-purple-500/10 px-1.5 py-0.5 rounded border border-purple-500/20">
                                        <Volume2 size={7} />{ep.audioLanguage}
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <button
                                      ...
                                      className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-all ${
                                        isLocked
                                          ...
                                          : 'bg-[#1a1a1a] hover:bg-[#252525] border border-white/10 text-gray-400 hover:text-white active:scale-95'
                                      }`}
                                    >
                                      {ep.downloadLink ? <ExternalLink size={11} /> : <Download size={11} />}
                                      <span>DL</span>
                                    </button>
                                    ...
                                    <button
                                      ...
                                      disabled={isLocked}
                                      className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${
                                        isLocked
                                          ? 'bg-gray-800 border border-gray-700 cursor-not-allowed opacity-40 text-gray-600'
                                          : 'bg-gold/15 hover:bg-gold/25 border border-gold/30 text-gold active:scale-95'
                                      }`}
                                    >
                                      <Play size={11} fill={isLocked ? 'gray' : '#FFD700'} />
                                      <span>Watch</span>
                                    </button>
                                  </div>
                                </div>
